...
default_passages = [
    p for p in ["Bab-Al Mandab Strait", "Suez", "Cape of Good Hope"]
    if p in PASSAGES
]
...
with col_qbar:
    ...
    fig_qbar = px.bar(
        qbar_data,
        x="quarter", y="Number of crossings", color="Passage",
...
if not suez_monthly_df.empty:
    ...
    fig_corr.add_trace(
        go.Bar(
            x=merged["month"], y=merged["yemen_events"], name="Yemen Events",
    ...
    fig_corr.add_trace(
        go.Scatter(
            x=merged["month"], y=merged["suez_crossings"], name="Suez Crossings",
